...
: RSA server.pem # Private key of server's cert
: PSK "$VPN_PSK" # Pre-shared key
$VPN_USER : EAP   "$VPN_PASSWORD" # Username and pw for EAP
$VPN_USER : XAUTH "$VPN_PASSWORD" # Username and pw for XAUTH
...
ipsec pki --self \
          ...
          --dn "C=UK, O=strongSwan, CN=strongSwan Root CA" \
          --ca \
          --lifetime 3650 \
...
ipsec pki --pub --in /etc/ipsec.d/private/server.pem |
    ipsec pki --issue --lifetime 1200 --cacert /etc/ipsec.d/cacerts/ca.cert.pem \
              --cakey /etc/ipsec.d/private/ca.pem --dn "C=UK, O=strongSwan, CN=${VPN_DOMAIN}" \
...
ipsec pki --pub --in /etc/ipsec.d/private/client.pem |
    ipsec pki --issue \
              ...
              --cakey /etc/ipsec.d/private/ca.pem --dn "C=UK, O=strongSwan, CN=client@${VPN_DOMAIN}" \
...
$(base64 /etc/ipsec.d/client.cert.p12)
   ...
   <string>Add PKCS#12 format Cert</string>
...
$(base64 /etc/ipsec.d/certs/server.cert.pem)
   ...
   <key>PayloadDescription</key>
   <string>Add PKCS#1 format Cert</string>
   <key>PayloadDisplayName</key>
   <string>${VPN_DOMAIN}</string>
   <key>PayloadIdentifier</key>
